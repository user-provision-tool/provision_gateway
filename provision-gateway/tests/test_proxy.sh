...
R=$(curl -s -b "$COOKIE_JAR" "$GW/api/system/proxy")
...
R=$(curl -s -X POST -b "$COOKIE_JAR" "$GW/api/system/proxy" \
    -H "Content-Type: application/json" \
...
R=$(curl -s -X PUT -b "$COOKIE_JAR" "$GW/api/system/proxy/$PROXY_ID" \
    -H "Content-Type: application/json" \
...
R=$(curl -s -X PUT -b "$COOKIE_JAR" "$GW/api/system/proxy/$PROXY_ID/activate")
...
R=$(curl -s -X POST -b "$COOKIE_JAR" "$GW/api/system/proxy/test")
...
R2=$(curl -s -b "$COOKIE_JAR" "$GW/api/system/proxy")
...
R=$(curl -s -X POST -b "$COOKIE_JAR" "$GW/api/system/proxy/deactivate")
...
R=$(curl -s -X DELETE -b "$COOKIE_JAR" "$GW/api/system/proxy/$PROXY_ID")
...
R=$(curl -s -b "$COOKIE_JAR" "$GW/api/audit?action=proxy_config")
